...
export async function loadCategoryPage(categorySlug: string) {
  ...
  const { data: topSellers } = await supabase
    .rpc('get_top_category_sellers', { category_uuid: category.id }) as { data: any[] | null };
...
export async function loadSubcategoryPage(categorySlug: string, subcategorySlug: string, url: URL) {
  // Get category info
